	...
	<div class="row">
		<div class="col-xl-3 col-md-6">
			<div class="card bg-primary text-white mb-4">
				<div class="card-body">
					<p>{{ _lang('Total Users') }}</p>
					<h5 class="mt-2">{{ $total_user_count }}</h5>
				</div>
			</div>
		</div>
		<div class="col-xl-3 col-md-6">
				    ...
				    <table class="table table-striped data-table">
						...
						<tbody>
							...
							<tr>
								...
								<td class="text-right {{ $transaction->dr_cr == 'cr' ? 'text-green' : 'text-red' }} {{ $transaction->status == 'reject' ? 'text-rejected' : '' }}"><b>{{ decimalPlace($transaction->amount, $currency) }}</b></td>
